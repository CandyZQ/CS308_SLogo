...
public class Turtle implements MovingObject {
  ...
  /**
   * Turtle.java is updated from Controller according to commands, and returns TurtleState object as
   * needed for View to update display/track history.
   */
  public Turtle(Integer ID) {
    ...
    this.stateMap.put(MovingObjectProperties.VISIBILITY,
        defaultStateMap.get(MovingObjectProperties.VISIBILITY));
    this.stateMap.put(MovingObjectProperties.PEN, defaultStateMap.get(MovingObjectProperties.PEN));
    this.stateMap.put(MovingObjectProperties.RETURN_VALUE,
        defaultStateMap.get(MovingObjectProperties.RETURN_VALUE));
    this.stateMap
        .put(MovingObjectProperties.CLEAR, defaultStateMap.get(MovingObjectProperties.CLEAR));
  }
  ...
  /**
   * Sets (x,y) coordinates of Turtle's current state. Assumes grid center (0,0).
   *
  ...
  public double setCoordinates(double x, double y) {
    double distance = findDistance((double) stateMap.get(MovingObjectProperties.X),
        (double) stateMap.get(MovingObjectProperties.Y), x, y);
  ...
  /**
   * Sets ABSOLUTE heading of Turtle's current state in degrees. Assumes north of (0,0) is 0
   * degrees. Turning clockwise from NORTH increases angle such that: Heading EAST is 90 degrees.
   * Heading WEST is 270 degrees. ABSOLUTE - if current heading is 90 and new heading is 95, the
   * total rotation is 5, not 95
   *
   ...
   * @return absolute value of number of degrees moved, assume when heading set turtle always moves
   * COUNTERCLOCKWISE
   */
  @Override
  ...
  public double moveDistance(double distance) {
    ...
    double opposite = hypotenuse * Math.sin(Math.toRadians(
        getCoterminal((double) this.stateMap.get(MovingObjectProperties.HEADING), POS_ANGLE)));
    double adjacent = hypotenuse * Math.cos(Math.toRadians(
        getCoterminal((double) this.stateMap.get(MovingObjectProperties.HEADING), POS_ANGLE)));
  ...
  private double getRotation(double currentHeading, double newHeading) {
    ...
    return Math
        .abs(getCoterminal(currentHeading, POS_ANGLE) - getCoterminal(currentHeading, POS_ANGLE));
  }
  ...
  private double[] getDeltas(double distance, double currentHeading, double opposite,
      double adjacent) {
    double[] delta = {0, 0};
